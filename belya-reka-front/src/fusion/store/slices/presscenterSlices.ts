...
// Изменяем thunk: теперь первым аргументом идет locale (string)
export const getPrssCntr = createAsyncThunk("presscenter/getPrssCntr", async (locale: string = "ru", { rejectWithValue }) => {
  try {
    // Подставляем динамическую локаль в запрос
    const response = await $api.get(`/news?populate=*&sort=createdAt:desc&locale=${locale}`);
