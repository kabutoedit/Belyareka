      ...
      <LazyLoadLayout>
        <ContainerLayout className="relative z-10 pt-8 pb-24">
          <div className="relative z-10 flex flex-col">
            ...
            <div className="pt-8 flex flex-col md:flex-row md:justify-between xl:flex-nowrap flex-wrap xl:gap-0 gap-y-4 z-50">
              <div>
                <span className="text-base">© 2025-2030 {t("footer.company_name")}</span>
                <p className="xl:text-xs text-base underline cursor-pointer">{t("footer.privacy_policy")}</p>
              </div>
              ...
              <div className="flex gap-x-6 xl:flex-row flex-col xl:gap-y-0 gap-y-3">
                {navLink?.map((nav) => (
                  <Link
                    key={nav.link}
                    className="no-underline text-white xl:text-sm text-lg xl:font-normal font-semibold h-max hover:opacity-80 transition-opacity"
                    to={nav.link}>
                    {nav.title}
                  </Link>
                ))}
