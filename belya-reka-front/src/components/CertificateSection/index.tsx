...
const CertificateMainSection: FC = () => {
  const { t, i18n } = useTranslation(); // Достаем i18n
  const isMobile = useMediaQuery("(max-width: 1023.98px)");
  const [isOpen, setIsOpen] = useState(false);
  const [data, setData] = useState<CertificateData | null>(null);

  useEffect(() => {
    // Используем i18n.language для динамического запроса
    // Если в i18next код 'kgz', а в Strapi 'ky', используем условие:
    const currentLocale = i18n.language === "kgz" ? "ky" : i18n.language;

    $api
      .get(`/certificate-block?populate=*&locale=${currentLocale}`)
      .then((response) => {
        // Проверяем наличие данных, так как в Strapi 4 структура может быть разной
        const result = response.data.data?.attributes || response.data.data;
        ...
        if (result) {
          setData({
            ...
            description: result.description || "",
            images:
              result.images?.data?.map((img: any) => ({
                id: img.id,
                url: img.attributes?.url || img.url,
                name: img.attributes?.name || img.name,
              })) || [],
          });
        }
      })
  ...
  return (
    <ContainerLayout className="mt-20">
      ...
      {/* Заголовки теперь приходят из Strapi на нужном языке */}
      <div className="flex justify-between items-end mt-8 2xl:flex-nowrap flex-wrap">
        <span className="br-text-medium block leading-7 2xl:basis-3/4 w-full text-balance">{data?.title}</span>
        <p className="2xl:mt-0 mt-5 text-lg w-full text-balance">{data?.description}</p>
      </div>
      ...
      <LazyLoadLayout>
        <Swiper
          ...
          modules={[Navigation]}>
          {certificates?.map((item, index) => (
            <SwiperSlide key={index} className="swiper-slide-custom" onClick={() => setIsOpen(true)}>
              <LazyLoadImage
                wrapperClassName="w-full"
                className="certificate__img--block bg-snow p-5 rounded-2xl"
                src={item.src}
