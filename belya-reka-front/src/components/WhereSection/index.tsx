...
const WhereBlock: FC = () => {
  ...
  const [coor, setCoor] = useState<number[]>(DEFAULT_CENTER);
  // Используем ключ перевода для начального значения "Все"
  const [titleArea, setTitleArea] = useState<string>("all");
  const [activeRaions, setActiveRaions] = useState<any[]>([]);
  const [titleRaion, setTitleRaion] = useState<string>("");
  const [partners, setPartners] = useState<Partner[]>([]);

  // Маппинг языков i18next на формат Яндекс.Карт
  ...
  return (
    <section>
      ...
      <div className="mt-8 w-full h-[500px]">
        <LazyLoadLayout>
          {/* Динамически меняем язык карт */}
          <YMaps query={{ lang: mapLang, load: "package.full" }}>
            <Map
              width="100%"
              height="500px"
              defaultState={{ center: DEFAULT_CENTER, zoom: 7 }}
