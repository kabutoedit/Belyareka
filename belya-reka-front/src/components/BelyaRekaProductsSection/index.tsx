...
// Добавили аргумент locale
async function fetchAllProducts(locale: string): Promise<IProductCatalog[]> {
  const pageSize = 100;
  // В Strapi для кыргызского обычно используется 'ky', если у тебя 'kgz', добавим проверку
  const strapiLocale = locale === "kgz" ? "ky" : locale;

  const res = await fetch(`${STRAPI_URL}/api/products?locale=${strapiLocale}&populate=category&populate=image&pagination[pageSize]=${pageSize}`);
  ...
  return json.data.map((item: any) => {
    ...
    return {
      id: String(item.id),
      title: a.name ?? "", // Теперь тут будет имя на нужном языке
      img: imageUrl,
      imgns: imageUrl,
      className: a.className ?? "",
      isShow: a.isShow ?? false,
      bg: a.bg ?? "",
      category: categoryName, // Категория тоже придет переведенная, если она локализована в Strapi
      textColor: undefined,
      props_product: {
        life_cycle: a.life_cycle ?? 10,
        weight: a.weight ?? "",
        volume: a.volume ?? "",
        temperature_keep: a.temperature ?? "+2°C до +6°C",
      },
      price: a.price ?? 0,
      old_price: 149,
      starting_price: 86,
...
const BelyaRekaProductsSection: FC = () => {
  const { t, i18n } = useTranslation(); // Достаем i18n для отслеживания языка
  const [allProducts, setAllProducts] = useState<IProductCatalog[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    setLoading(true);
    fetchAllProducts(i18n.language).then((data) => {
      setAllProducts(data);
      setLoading(false);
    });
  }, [i18n.language]); // Следим за языком
  ...
  }, [activeProductIndex, knowYourPlaceState, controls]);
