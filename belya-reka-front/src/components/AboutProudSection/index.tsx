...
const AboutProudSection: FC = () => {
  ...
  useEffect(() => {
    // Добавляем параметр locale, чтобы Strapi фильтровал контент
    $api
      .get(`/proud-items?populate=*&locale=${i18n.language}&sort=order:asc`)
      .then((res) => {
        const strapiData = res.data.data;
  ...
  }, [i18n.language]); // Следим за сменой языка
    ...
    <section className="mt-28">
      <LazyLoadLayout>
        <div className="">
          <div className="lg:w-1/3 flex-grow-0">
            <LegendBelyaReka className="xl:mb-24 mb-10 block">
              {/* Локализованный заголовок из JSON */}
              {t("about.proud_title_1")} <br />
              {t("about.proud_title_2")}
            </LegendBelyaReka>
          </div>
