...
const AboutValuesSection: FC = () => {
  ...
  useEffect(() => {
    // Подставляем i18n.language для фильтрации контента в Strapi
    $api
      .get(`/company-values?populate=*&locale=${i18n.language}&sort=order:asc`)
      .then((res) => {
        const strapiData = res.data.data;
  ...
  }, [i18n.language]); // Перезапрос при смене языка
