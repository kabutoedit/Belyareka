...
const HeaderPresentation: FC = () => {
  ...
  const slides = strapiSlides.length > 0 ? strapiSlides : null;
  const elementText = slides ? slides.map((s) => ({ title: s.title, description: s.description })) : localizedFallbackText;
  const backgroundColor = slides ? slides.map((s) => s.bgColor || "#1A7BD3") : fallbackBackgroundColor;
  ...
  useEffect(() => {
    // Добавляем locale в запрос для получения переводов из Strapi
    $api
      .get(`/home-page?populate[slides][populate][0]=productImageDesktop&populate[slides][populate][1]=productImageMobile&locale=${i18n.language}`)
      .then((res) => {
        const data = res.data?.data;
        const attributes = data?.attributes || data;
        const slidesData = attributes?.slides;
        ...
        if (slidesData && Array.isArray(slidesData) && slidesData.length > 0) {
          ...
          const normalizedSlides = slidesData.map((slide: any) => {
            ...
            const desktopImg = slide.productImageDesktop;
            if (desktopImg) {
              if (desktopImg.url) desktopUrl = desktopImg.url;
              else if (desktopImg.data?.attributes?.url) desktopUrl = desktopImg.data.attributes.url;
            }

            let mobileUrl = "";
            const mobileImg = slide.productImageMobile;
            if (mobileImg) {
              if (mobileImg.url) mobileUrl = mobileImg.url;
              else if (mobileImg.data?.attributes?.url) mobileUrl = mobileImg.data.attributes.url;
            }
