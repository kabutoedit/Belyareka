...
const ContactsSection: FC = () => {
      ...
      try {
        const response = await $api.get("/contact-departments", {
          params: {
            "populate[contact_cards][populate][0]": "contacts",
            "sort[0]": "order:asc",
            locale: i18n.language, // Добавляем локаль в запрос
          },
        });

        const departments = response.data.data as StrapiDepartment[];
        setTabData(transformDepartments(departments));
      } catch (error) {
        console.error("Ошибка загрузки отделов:", error);
        setTabData([]);
      } finally {
  ...
  const activeCategory = tabData.length > 0 ? tabData[tabIndex]?.title : t("contacts.no_department");
  const hasTabs = tabData.length > 0;

  // Общий рендер баннера для всех состояний
  const renderBanner = () => (
    <BannerCommon
      banner={banner}
      bannerTitle={t("navbar.contacts")}
      bannerDescription={
        <p className="text-white md:text-2xl text-lg text-balance block">
          {t("contacts.banner_desc_1")} <br /> {t("contacts.banner_desc_2")}
        </p>
      }
    />
  );
