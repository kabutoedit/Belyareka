...
const ContactsSectionForm: FC<ContactsFormProps> = ({ category }) => {
  ...
  const onSubmithandler = async (data: FormInputs) => {
    ...
    const promise = $api.post("/contact-requests", {
      data: {
        ...
        email: data.email,
        message: data.message,
        department: category ?? t("contacts.default_dept"),
      },
    });
    ...
    toast.promise(promise, {
      ...
      success: () => {
        setIsLoading(false);
        reset();
        return t("contacts.success_msg");
      },
      error: () => {
        setIsLoading(false);
        return t("contacts.error_msg");
      },
    });
  };
  ...
  return (
    <>
      ...
      <div className="w-full flex flex-col xl:flex-row gap-5 justify-between">
        <div className="w-full">
          <h2 className="text-4xl font-bold text-jetblack font-cocosignum">{t("contacts.title")}</h2>
          <p className="text-lg w-11/12 mt-2">{t("contacts.description")}</p>
        </div>
        <form className="flex flex-col w-full gap-5" onSubmit={handleSubmit(onSubmithandler)}>
          <InputBasketUi
            className="h-[50px]"
            error={!!errors?.name}
            value={watch("name")}
            {...register("name", { required: t("contacts.required_name") })}
          ...
          <InputBasketUi
            ...
            value={watch("email")}
            {...register("email", {
              required: t("contacts.required_email"),
              pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: t("contacts.invalid_email") },
            })}
          ...
          <InputBasketUi
            ...
            error={!!errors?.message}
            value={watch("message")}
            {...register("message", { required: t("contacts.required_message") })}
