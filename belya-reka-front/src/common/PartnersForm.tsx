...
const PartnersForm: FC = () => {
  ...
  return (
    <section className="w-full flex flex-col xl:flex-row gap-5 justify-between pb-1">
      <div className="w-full">
        <h2 className="text-4xl font-bold text-jetblack font-cocosignum">{t("partners.title")}</h2>
        <p className="text-lg w-11/12 mt-2">{t("partners.description")}</p>
      </div>

      <form className="flex flex-col w-full gap-5" onSubmit={handleSubmit(onSubmithandler)}>
        <fieldset className="flex flex-col w-full gap-5">
          <Input label={t("contacts.name")} error={!!errors.name} value={watch("name")} {...register("name", { required: true })} name="name" />
          <Input label={t("partners.email")} error={!!errors.email} value={watch("email")} {...register("email", { required: true })} name="email" />
          <Controller
            name="phoneNumber"
            control={control}
            rules={{ required: true }}
        ...
        <fieldset className="flex gap-y-2 gap-x-4 md:items-center flex-col md:flex-row">
          <button type="submit" disabled={isLoading} className="bg-hexahrome py-3 rounded-full text-white w-[200px]">
            {isLoading ? t("common.loading") : t("common.send")}
          </button>
          <p className="text-sm">{t("partners.confidential")}</p>
        </fieldset>
      </form>
    </section>
  );
