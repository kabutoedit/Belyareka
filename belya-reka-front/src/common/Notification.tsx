...
// CDN Base URL for DigitalOcean Spaces
const CDN_BASE = "https://belaya-reka-storage.fra1.digitaloceanspaces.com";
const errorIcon = `${CDN_BASE}/assets/media/svg/error.svg`;
...
const Notification: React.FC<NotificationProps> = ({ message, variant, onClose, onOkay, navigate, timerIfAction, navigateText, showPage }) => {
  ...
  }, [navigate, navigateText, startCountdown]);
  ...
  }, [onClose, onOkay, navigate, navigateIFSuccess, timerIfAction, showPage]);

  const handleOkay = () => onOkay?.();
      ...
      <AnimatePresence>
        <motion.div
          ...
          <div
            ...
            <div className={`flex items-center py-6 px-5 md:pl-5 gap-6 flex-col md:flex-row text-center md:text-left`}>
              <div>{variant === "success" ? successSvg : <img src={errorIcon} alt="error icon" />}</div>

              <div>
                <p className="text-base leading-5 text-jetblack font-normal max-w-[636px]">{message}</p>
                {navigate && navigateText && <p className="mt-1">{t("notifications.redirect", { target: navigateText, count: count })}</p>}
              </div>
            </div>
