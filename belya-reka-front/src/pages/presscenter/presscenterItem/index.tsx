...
const PressCenterElem: FC = () => {
  ...
  useEffect(() => {
    // Перезагружаем список новостей при смене языка, чтобы получить контент на нужном языке
  ...
  if (!newsItem && loading === "idle") {
    return (
      <ContainerLayout className="pt-40 text-center">
        <div>{t("press_center.not_found")}</div>
        <button onClick={() => navigate(-1)} className="mt-4 text-blue-500 underline">
          {t("common.back")}
        </button>
      </ContainerLayout>
    ...
    <section>
      ...
      <ContainerLayout>
        ...
        {otherNews && otherNews.length > 0 && (
          <div className="mt-20 pt-10 border-t border-gray-200">
            <h2 className="text-3xl font-bold mb-8 font-cocosignum text-left">{t("press_center.read_also")}</h2>
            <div className="grid xl:grid-cols-3 md:grid-cols-2 gap-6">
              {otherNews.map((item) => (
                <ArticleItemCommon key={item.id} {...item} />
              ))}
